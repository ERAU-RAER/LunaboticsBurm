...
class SerialDrivetrainNode(Node):
    def __init__(self):
        super().__init__('serial_drivetrain_node')
        ...
        self.serial_pub = self.create_publisher(String, '/drive_chatter', 10)
...
def main(args=None):
    ...
    serial_drivetrain_node = SerialDrivetrainNode()
    
    # Start a separate thread to continuously read serial messages
    serial_thread = threading.Thread(target=serial_drivetrain_node.read_serial)
    serial_thread.start()
    
    try:
        rclpy.spin(serial_drivetrain_node)
    except KeyboardInterrupt:
        pass
    
    serial_drivetrain_node.destroy_node()
    rclpy.shutdown()
